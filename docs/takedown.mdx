...
![](https://github.com/urldna/docs/blob/main/images/docs/takedown.png?raw=true)
...
graph TD;
    ...
    D --> DA[Takedown Case Status: Open]
    DA --> E[IP Abuse Report]
    DA --> F[Domain Abuse Report]
    DA --> G[Google Safe Browsing Report]
    E & F & G --> H[Takedown Case Status: Pending Takedown]
    H --> I[View Takedown Case on Scan Page]
    I --> J[Rescan URL]
    J -->|URL Inaccessible/Taken Down| K[Close Takedown Case]
    K --> L[Takedown Case Status: Close]
...
* After completing these steps, the takedown case status is set to **Pending Takedown**.
